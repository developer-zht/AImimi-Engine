    ...
    for (int i = 0; i < 6; i++) {
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                // Edit Start
                auto deltaOmega = CalcArea(x, y, width, height);

                auto omega = Eigen::Vector3d(dir.x(), dir.y(), dir.z()).normalized();

                for (int l = 0; l <= SHOrder; l++) {
                    for (int m = -l; m <= l; m++) {
                        double shBase = sh::EvalSH(l, m, omega);
                        int index = sh::GetIndex(l, m);
                        SHCoeffiecents[index] += Le * shBase * deltaOmega;
                    }
                }
                // Edit End
            }
...
class PRTIntegrator : public Integrator {
  public:
    ...
    std::unique_ptr<std::vector<double>> computeInterreflectionSH(const Eigen::MatrixXf &directTSHCoeffs, const Point3f &pos, const Normal3f &normal, const Scene *scene, int bounces) {
        ...
        for (int t = 0; t < sample_side; t++) {
            for (int p = 0; p < sample_side; p++) {
                ...
                nori::Ray3f ray = Ray3f(pos, wi.normalized());
                Intersection its;

                if (H > 0 && scene->rayIntersect(ray, its)) {
                    MatrixXf normals = its.mesh->getVertexNormals();
                    Point3f idx = its.tri_index;
                    Point3f hitPos = its.p;
                    Vector3f bary = its.bary;

                    Normal3f hitNormal = Normal3f(normals.col(idx.x()).normalized() * bary.x() + normals.col(idx.y()).normalized() * bary.y() + normals.col(idx.z()) * bary.z()).normalized();

                    auto hitIndirectCoeff = computeInterreflectionSH(directTSHCoeffs, hitPos, hitNormal, scene, bounces + 1);

                    for (int j = 0; j < SHCoeffLength; j++) {
                        auto interpolateSH = directTSHCoeffs.col(idx.x()).coeffRef(j) * bary.x() + directTSHCoeffs.col(idx.y()).coeffRef(j) * bary.y() + directTSHCoeffs.col(idx.z()).coeffRef(j) * bary.z();

                        // H 是 Lambert 余弦衰减，但我认为不需要 * H，因为 directTSHCoeffs 中已经 * H 了
                        // (*coeffs)[j] += (interpolateSH + (*hitIndirectCoeff)[j]) * H;
                        (*coeffs)[j] += (interpolateSH + (*hitIndirectCoeff)[j]);
                    }
                }
            }
        }
    ...
    virtual void preprocess(const Scene *scene) override {
        ...
        std::filesystem::path meshPath(mesh->getName());
        std::string meshName = meshPath.stem().string();
        // Projection environment
        auto cubePath = getFileResolver()->resolve(m_CubemapPath);
        std::string envName = cubePath.filename();

        std::string typeStr;
        switch (m_Type) {
            case Type::Unshadowed:
                typeStr = "unshadowed";
                break;
            case Type::Shadowed:
                typeStr = "shadowed";
                break;
            case Type::Interreflection:
                typeStr = "interreflection";
                break;
        }
        ...
        auto lightPath = cubePath / ("light.txt");
        auto transPath = cubePath / ("transport_" + meshName + "_" + typeStr + ".txt");
        ...
        for (int i = 0; i < mesh->getVertexCount(); i++) {
            ...
            auto shFunc = [&](double phi, double theta) -> double {
                ...
                if (m_Type == Type::Unshadowed) {
                    ...
                    nori::Ray3f ray = Ray3f(v, wi.normalized());
            ...
            // 假设能从mesh获取每个顶点的albedo
            // Color3f albedo = mesh->getVertexAlbedo(i);
            for (int j = 0; j < shCoeff->size(); j++) {
                // Edit Start
                // m_TransportSHCoeffs.col(i).coeffRef(j) = (*shCoeff)[j];
                // albedo = 1, albedo / pi = 1 / M_PI，即 brdf 系数
                m_TransportSHCoeffs.col(i).coeffRef(j) = (*shCoeff)[j] / M_PI;
                // Edit End

                // m_TransportSHCoeffs_R.col(i).coeffRef(j) = (*shCoeff)[j] * albedo.r() / M_PI;
                // m_TransportSHCoeffs_G.col(i).coeffRef(j) = (*shCoeff)[j] * albedo.g() / M_PI;
                // m_TransportSHCoeffs_B.col(i).coeffRef(j) = (*shCoeff)[j] * albedo.b() / M_PI;
            }
        }
        if (m_Type == Type::Interreflection) {
            const int vertexCount = mesh->getVertexCount();
            // 临时矩阵，目的是保证原系数表 m_TransportSHCoeffs 在遍历（更新）过程中的不变，避免相互照亮的点之间更新系数时因先后顺序造成影响
            // 例如：假设顶点 A（索引 100）和顶点 B（索引 200）互相照亮：
            // 处理顶点 A 时（i = 100）：去查 B 的系数，此时 B 还没被更新过，拿到的是 B 的纯直接光照系数 TB0T_B ^ 0 TB0​。✅ 正确
            // 处理顶点 B 时（i = 200）：去查 A 的系数，但此时 A 已经被更新成了 T_A ^ 0 + \Delta_A​（带间接光照的值）。❌ 不一致
            Eigen::MatrixXf indirectAll(SHCoeffLength, vertexCount);
            indirectAll.setZero();
            ...
            for (int i = 0; i < vertexCount; i++) {
                ...
                for (int j = 0; j < SHCoeffLength; j++) {
                    indirectAll.col(i).coeffRef(j) = (*indirectCoeffs)[j];
                }
            }
            m_TransportSHCoeffs += indirectAll; // 全部算完后再统一加
        }
        ...
        std::cout << "Computed transport sh coeffs from: " << cubePath.str()
