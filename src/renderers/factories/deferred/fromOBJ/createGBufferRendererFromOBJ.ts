...
import { GBufferRendererFromOBJConfig } from './types/createGBufferRendererFromOBJ'

/**
 * OBJMeshData → GBuffer MeshRenderer
 *
 * 职责：
 *   单个 OBJMeshData（纯数据）→ Mesh + GBufferMaterial + GBuffer Shader → MeshRenderer
 *
 * 不负责：
 *   - 加载 OBJ 文件（由调用方 loadOBJ 完成）
 *   - 遍历多个 mesh group（由调用方循环完成）
 *   - Shadow uniform 推送（由 ShadowRenderPass 自动推送）
 *
 * Shader 路径固定为 GBuffer shader，不需要调用方传入。
 * 因为 GBuffer 的输出格式（gl_FragData[0-4]）是固定的，
 * 不存在"用不同 shader 写 GBuffer"的场景。
 */
export async function createGBufferRendererFromOBJ(
  gl: WebGLRenderingContext,
  config: GBufferRendererFromOBJConfig
): Promise<MeshRenderer> {
  const { data, rendererName } = config
  const ctx = '[createGBufferRendererFromOBJ]'
  ...
  // GBufferMaterial 内部已经声明了所有 shadow 占位 uniform：
  //   uKd, uNt,
  //   uShadowMap(null), uLightVP(identity), uUseDepthTexture(0),
  //   uLightPos, uShadowMapSize, uFrustumSize, uLightNearPlane, uLightWorldSize
  //
  // 这些占位值会被 ShadowRenderPass.applyStaticUniforms() 和 applyPerFrameUniforms() 覆盖为真实值
  ...
  // 3. 创建 Shader
  // 路径固定：所有 GBuffer renderer 用同一套 shader
  // Shader.createShader 内部有缓存——如果这个 shader 已经编译过，
  // 第二次调用直接返回缓存的实例，不会重复编译
 ...
 */
function createGBufferMesh(gl: WebGLRenderingContext, data: OBJMeshData, label: string): Mesh {
  ...
  if (data.uvs) {
    attributes.push({ name: 'aTextureCoord', array: data.uvs, size: 2, type: gl.FLOAT })
  } else {
    console.warn(`${label} Model "${data.name}" has no UVs, textures will not sample correctly`)
  }

  if (data.tangents) {
    attributes.push({ name: 'aTangent', array: data.tangents, size: 4, type: gl.FLOAT })
  } else {
