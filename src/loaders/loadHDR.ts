...
export class HDRTextureLoader {
  ...
  private async loadEXRFile(filePath: string) {
    ...
    try {
      ...
      const internalFormat = externalFormat
      console.log(`🔘 DataTexture: ${dataTexture}`)
      const exrTexture = this.gl.createTexture()
      if (!exrTexture) {
        throw new Error('❌ 无法创建 WebGL 纹理对象')
      }
      this.gl.bindTexture(this.gl.TEXTURE_2D, exrTexture)
      this.gl.texImage2D(
        this.gl.TEXTURE_2D,
        0,
        internalFormat,
  ...
  public async loadHDRTexture(filePath: string, fileType: TextureFileType) {
    switch (fileType) {
      case TextureFileType.HDR:
        return await this.loadHDRFile(filePath)
      case TextureFileType.EXR:
        return await this.loadEXRFile(filePath)
      default:
